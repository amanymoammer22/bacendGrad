...
exports.createCashOrder = asyncHandler(async (req, res, next) => {
    ...
    const order = await Order.create({
        ...
        paymentMethodType: req.body.paymentMethod || "cash",
        totalOrderPrice,
    });
...
exports.findAllOrders = factory.getAll(Order, "Order", [
    { path: "user", select: "firstName secondName email phone" },
...
exports.checkoutSession = asyncHandler(async (req, res, next) => {
    ...
    const session = await stripe.checkout.sessions.create({
        line_items: [
            {
                price_data: {
                    currency: "usd",
                    product_data: {
                        name: `Order for ${req.user.firstName} ${req.user.secondName}`,
                    },
        ...
        mode: "payment",
        success_url: "http://localhost:5173/successPayPage",
        cancel_url: "http://localhost:5173/paymentFailedPage",
...
exports.webhookCheckout = (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
        console.log("💳 Checkout completed, creating order...");
